...
class QnAEngine:
            ...
            for start_line, currline in enumerate(alllines):
                ...
                if len(currline.strip()) > 0:
                    ...
                    if found and len(found.group(2).strip()) > 0:
                        if (
                            len(segmentlines) > 1 and len(segmentlines[0].strip()) > 0
                        ) or (mddict[found.group(1)] == "H2" and "H2" in metalist):
                            # more than 1 line before or new H2 and previousely was H2

                            if (
                                found.group(2).strip() != txt2add
                            ):  # new chunk if not only title but other text as well
                        ...
                        if found:
                            if (
                                found.group(1) in othermeta and len(txt2add) > 0
                            ):  # new value for existing tag
                                if found.group(1) != "pg":
                                    newmeta = {**metalist, **othermeta}
                                    docs.append(
                                        Document(text=txt2add, extra_info=newmeta)
                                    )
                                    segmentlines.clear()
                    ...
                    #    segmentlines.append(currline)
                        ...
                        if found:
                            ...
                            if (
                                len(segmentlines) > 1
                                and len(segmentlines[0].strip()) > 0
                            ) or (title == "H2" and "H2" in metalist):
                                # more than 1 line before or new H2 and previousely was H2
    ...
    async def get_nodes(
        self, documents, filetype, chunk_size, chunk_overlap
    ) -> List[TextNode]:
        ...
        for doc in documents:
            ...
            currtext = doc.text
            currtext = re.sub(
                r"(\*\*SATURS\*\*)([^\*]+)", r"", currtext, flags=re.IGNORECASE
            )  # Remove content, no usefull information for the RAG
            if len(currtext) > 0:
                ...
                for idx, chunk in enumerate(cur_text_chunks):
                    ...
                    if (
                        n > 0
                        and nodes[-1].metadata.get("H1", "")
                        == node.metadata.get("H1", "")
                        and nodes[-1].metadata.get("H2", "")
                        == node.metadata.get("H2", "")
                        and nodes[-1].metadata.get("H3", "")
                        == node.metadata.get("H3", "")
                    ):
    ...
    async def createIndex(
        ...
        try:
            ...
            self.newindex = VectorStoreIndex(
                ...
                embed_model=self.embeddingobject,
            )
    ...
    def compressPrompt(self, prompt, size):
        if prompt in self.compressed_txt:
            ...
            self._token_counter.estimate_tokens_in_messages(
                [ChatMessage(content=prompt, role=MessageRole.SYSTEM)]
            )
            < size
        ):
            return prompt
        ...
        response = self.llm.complete(
            f"The following text exceeds context window token limit. Summarize it so its size does not exceed {size} tokens. Do not translate the text. The text is the following:\n{prompt}\nThe summaized text: "
        )
        newprompt = str(response)
        newprompt = re.sub(
            r"The summarized text is( as follows)?:", "", newprompt
        ).strip()
            ...
            try:
                if n4rerank > 0:
                    ...
                    nodedict = self.getSimilarNodes(
                        q, n, prevnext
                    )  # {"text":texts, "score":scores, "metadata":metadata}
                ...
                print(
                    f"An exception in askQuestion: {type(error).__name__} {error.args[0]}"
                )
                return ""
        else:
            try:
                fullquery = PromptTemplate(
                    query_prompt + "\n" + text_qa_template_str
    ...
    def getRerankedNodes(self, q, n4rerank, n, prevnext=False):
        similarnodes = self.getSimilarNodes(
            q, n4rerank
        )  # {"text":texts, "score":scores, "metadata":metadata}
    ...
    def getSimilarNodes(self, q, n=4, prevnext=False):
        ...
        try:
            ...
            for item in retrieved_nodes:
                tmpdict = {
                    keys: values
                    for keys, values in item.metadata.items()
                ...
                if prevnext == True:
                    if "PrevNodeTxt" in tmpdict and tmpdict["PrevNodeTxt"] not in texts:
                        ...
                        scores.append(
                            item.get_score()
                        )  # use same score as for the next chunk
                ...
                if prevnext == True:
                    if "NextNodeTxt" in tmpdict and tmpdict["NextNodeTxt"] not in texts:
                        ...
                        scores.append(
                            item.get_score()
                        )  # use same score as for the previouse chunk
                        newtmpdict = tmpdict.copy()
                        newtmpdict.pop("NextNodeTxt", None)
                        newtmpdict.pop("PrevNodeTxt", None)
                        newtmpdict["ChunkNr"] = str(int(tmpdict["ChunkNr"]) + 1)
