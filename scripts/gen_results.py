...
def process_question(question_data, answer_data, qnaengine, embedding_conf, promptdict, supplementary_info, ofile, results_table, questions_process): 
    ...
    if 'questions' in question_data:
        for nested_question, nested_answer in zip(question_data.get('questions'),answer_data.get('answers')):
            if not questions_process or nested_question['nr'] in questions_process:
                process_question(nested_question, nested_answer, qnaengine, embedding_conf, promptdict,supplementary_info,ofile, results_table, questions_process)

def gen_results(qnaengine, configfile, embedding_conf, question_dictionary, answer_dictionary, promptdict, supplementary_info, questions_process): 
    results_table = []  
    # Generating log file
    with open("nodes.log", 'a', encoding='utf-8') as ofile: 
        ...
        for question, answer in zip(question_dictionary, answer_dictionary):
            if not questions_process or question['nr'] in questions_process:
                process_question(question, answer, qnaengine, embedding_conf, promptdict, supplementary_info, ofile, results_table, questions_process)
